...
#                    size, data_batch = self.create_data_batch()
                    ...
                    loss_d_r = self.D.loss(real_prediction, Variable(torch.ones(1)).cuda())
                    loss_d_r.backward()

                    generated_prediction = self.D(generated_batch)[0] # 1x1
                    loss_d_f = self.D.loss(1 - generated_prediction, Variable(torch.ones(1)).cuda())
                    loss_d_f.backward()
            ...
            for k in range(0, G_steps):
                ...
                generated_batch = self.G(Variable(z).cuda())

                D_prediction = self.D(generated_batch)[0] # 1x1
                loss_G = self.G.loss(D_prediction, Variable(torch.ones(1)).cuda())
                loss_G.backward()
